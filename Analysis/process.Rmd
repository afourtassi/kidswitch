...
d_sim_transform <-  d_sim_summary %>%
  mutate(normalized =ifelse(condition=='sound_real', mean/0.926, mean)) %>% #Here I only normalize sounds, objects are already normalized
  filter(distance != "iden") %>%
  mutate(stimuli = ifelse(condition =='obj_real','objects','sounds')) %>%
  mutate(normalized_all = ifelse(distance=="0.6", 1, normalized))%>% # Here I create a ligne for "different", which were not explicitly asked from participants because it was used as a prototype, thus dist=1
...
d_sim_transform$labels <- factor(d_sim_transform$labels, levels = c("similar","intermediate","different"))
#sum_child$sound_dist <- factor(sum_child$sound_dist, levels = c("similar", "different"))


feather::write_feather(d_sim_transform, "../saved_data/d_sim_transform.feather")  

ggplot(d_sim_transform, 
...
Now I need to predict accuracy as a function of sound and meaning similarity 
```{r}
...
fit_adults_2 <- nls(mean ~ combine_cues_2(s_dist, c_dist, Vr_s, Vr_o), data=subset(data_fit, group=='adults'), start=list(Vr_s=0.1, Vr_o=0.04), lower=c(0,0), algorithm="port")
...
model2_val <- add_rownames(as.data.frame(confint2(fit_children_2)), "param") %>%
  mutate(val= ifelse(param=='Vr_s',Vr_s_ch, Vr_o_ch),
         model=2, 
         group = "children") %>%
  bind_rows(add_rownames(as.data.frame(confint2(fit_adults_2)), "param") %>%
...
illustration <- data.frame(x=s, y=Gdist(s0=0, Vr=0.5, s), rep = "1", case = "low accuracy", type='Disambiguation') %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=0.5, Vr=0.5, s), rep = "2", case = "low accuracy", type='Disambiguation')) %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=0, Vr=0.5, s), rep = "1", case = "high accuracy", type='Disambiguation')) %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=1.1, Vr=0.5, s), rep = "2", case = "high accuracy", type='Disambiguation')) %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=0, Vr=0.5, s), rep = "1", case = "low accuracy", type='Development')) %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=0.5, Vr=0.5, s), rep = "2", case = "low accuracy", type='Development')) %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=0, Vr=0.1, s), rep = "1", case = "high accuracy", type='Development')) %>%
  bind_rows(data.frame(x=s, y=Gdist(s0=0.5, Vr=0.1, s), rep = "2", case = "high accuracy", type='Development'))
...
ggplot(illustration, aes(x=x, y=y, linetype=rep)) + 
  geom_line() + 
  theme_few()+
  theme(legend.title = element_text(size=11),
      legend.text=element_text(size=11),
  ...
  #geom_segment(aes(x = 0, y = 0, xend = 0, yend = y, colour = rep),
